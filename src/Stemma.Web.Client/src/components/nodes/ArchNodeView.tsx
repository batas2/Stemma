...
export function ArchNodeView({ id, data, selected }: NodeProps<ArchFlowNode>) {
  ...
  const animClass = anim !== 'none' && anim !== 'marching' ? `stemma-anim-${anim}` : '';
  if (animClass) {
  ...
  return (
    <>
      ...
      >
        {anim === 'marching' && (
          <div className="stemma-march" style={{ '--march': nodeStyle?.borderColor ?? 'rgb(99 102 241)' } as React.CSSProperties} />
        )}
