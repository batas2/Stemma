...
(function () {
  ...
  var data = JSON.parse(document.getElementById('stemma-report-data').textContent);
  var root = data.meta.rootPath;
  var LS = {
    state: 'stemma.report.state:' + root,
    drafts: 'stemma.report.drafts:' + root,
    author: 'stemma.report.author',
  };
  ...
  function exportPack() {
    ...
    var pack = {
      version: 1, kind: 'stemma-comment-pack',
    ...
    a.download = data.meta.workspace + '.comments.stemma.json';
    a.click();
  ...
  function buildShell() {
    ...
    var top = h('div', { class: 'vr-topbar' }, [
      h('span', { class: 'vr-logo' }, [logo, h('span', { text: 'Stemma' })]),
  ...
  function buildComments() {
    ...
    var bar = h('div', { style: 'display:flex;gap:8px;margin-bottom:16px;align-items:center' }, [
      ...
      h('span', { class: 'vr-hint', text: 'Import the pack in Stemma: Comments panel → Import pack.' }),
    ]);
