...
public sealed class YamlAdapterTests
{
    ...
    {
        var text = ReadFixture("minimal.stemma.yaml");
        var file = YamlConceptReader.Parse(FixturePath("minimal.stemma.yaml"), text);
    ...
    {
        var text = ReadFixture("realistic.stemma.yaml");
        var file = YamlConceptReader.Parse(FixturePath("realistic.stemma.yaml"), text);
    ...
    {
        var text = ReadFixture("pathological.stemma.yaml");
        var file = YamlConceptReader.Parse(FixturePath("pathological.stemma.yaml"), text);
    ...
    [Theory]
    [InlineData("minimal.stemma.yaml")]
    [InlineData("realistic.stemma.yaml")]
    [InlineData("pathological.stemma.yaml")]
    ...
    {
        var dir = Path.Combine(Path.GetTempPath(), "stemma-yaml-" + Guid.NewGuid());
        Directory.CreateDirectory(Path.Combine(dir, "Concepts"));
        File.WriteAllText(Path.Combine(dir, "Concepts", "data-model.stemma.yaml"), "concepts: []\n");
        try
        {
