...
export function TypeNodeView({ data, selected }: NodeProps<TypeFlowNode>) {
  ...
  return (
    <div
      className={clsx(
        'rounded-lg border bg-zinc-900/95 backdrop-blur shadow-lg min-w-[200px] max-w-[280px] transition-shadow',
        selected
    ...
    >
      ...
      <div className="px-3 py-2 border-b border-zinc-800 flex items-center gap-2">
        ...
        <span className="ml-auto font-medium text-zinc-100 text-sm truncate" title={t.name}>{t.name}</span>
      </div>
      {t.properties.length > 0 && (
        <ul className="text-[12px] py-1.5 text-zinc-300">
          {t.properties.slice(0, 6).map((p) => (
            <li key={p.name} className="px-3 py-0.5 flex justify-between gap-2 hover:bg-zinc-800/40">
