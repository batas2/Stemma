...
public class ArchModelTests
{
    private const string ProjectFile = """
        <Project Sdk="Microsoft.NET.Sdk">
          ...
          <ItemGroup>
            <ProjectReference Include="STEMMA_MODEL_PATH" />
          </ItemGroup>
        </Project>
        """;
    ...
    private static async Task<TestArchWorkspace> CreateAsync(string buildBody)
    {
        ...
        var csproj = ProjectFile.Replace("STEMMA_MODEL_PATH", modelProj);
        var arch = $$"""
            using Stemma.Model;
    ...
    public async Task MultiFile_reader_aggregates_elements_across_files()
    {
        ...
        var secondary = """
            ...
            """;
        var root = Path.Combine(Path.GetTempPath(), $"stemma-multi-{Guid.NewGuid():N}");
...
public sealed class TestArchWorkspace : IAsyncDisposable
{
    ...
    {
        var root = Path.Combine(Path.GetTempPath(), $"stemma-arch-{Guid.NewGuid():N}");
