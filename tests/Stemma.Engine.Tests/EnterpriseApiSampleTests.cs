...
public sealed class EnterpriseApiSampleTests
{
    ...
    public void Load_returns_three_yaml_files()
    {
        ...
        adapter.Files.Keys.Should().BeEquivalentTo("data-model.stemma.yaml", "resources.stemma.yaml", "view-book.stemma.yaml");
    }
    ...
    {
        var adapter = YamlAdapter.Load(SampleRoot());
        var data = adapter.Files["data-model.stemma.yaml"];
    ...
    {
        var adapter = YamlAdapter.Load(SampleRoot());
        var res = adapter.Files["resources.stemma.yaml"];
    ...
    {
        var adapter = YamlAdapter.Load(SampleRoot());
        var view = adapter.Files["view-book.stemma.yaml"];
