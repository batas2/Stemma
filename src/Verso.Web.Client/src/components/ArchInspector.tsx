  ...
  return (
    <aside className="w-[320px] shrink-0 border-l border-zinc-800 bg-zinc-950/60 flex flex-col overflow-hidden">
      ...
      <div className="px-4 py-3 border-b border-zinc-800 text-[11px] text-zinc-500 space-y-1">
        <div><span className="text-zinc-400">Kind</span> · {e.kind}</div>
        <div><span className="text-zinc-400">Id</span> · <span className="font-mono">{e.id}</span></div>
        {Object.entries(e.attributes).map(([k, v]) =>
          v !== null && v !== undefined ? (
            <div key={k}><span className="text-zinc-400">{k}</span> · <span className="font-mono">{v}</span></div>
          ) : null
        )}
      </div>
      <div className="p-3 border-t border-zinc-800 mt-auto">
